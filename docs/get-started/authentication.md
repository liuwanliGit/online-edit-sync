...
| 层级 | 凭据 | 持有者 | 用途 |
| --- | --- | --- | --- |
| **API Key** | `UMO_API_KEY`（环境变量） | 业务后端 | 调 `/api/token` 签发 JWT。**绝不暴露给前端** |
...
引擎的 `/api/token` 需要 `UMO_API_KEY`。如果前端直接持有 API Key，任何人都能签发任意 JWT 访问任意文档。

所以鉴权链路必须是：

1. **业务前端**调业务后端（走业务系统原有的登录态鉴权）
2. **业务后端**校验用户权限，决定角色（editor/viewer）
3. **业务后端**持 `UMO_API_KEY` 调引擎 `/api/token` 签发 JWT
4. **业务后端**把 JWT 返回给前端
...
router = APIRouter()
ENGINE_URL = "http://editor-host:9999"
UMO_API_KEY = "..."  # 从环境变量读
...
- **方案 B（被动刷新）**：协同连接断开时（`onAuthenticationFailed`），重新走「换 token → 重设 iframe src」流程

```js
// 被动刷新示例
window.addEventListener('message', (e) => {
  if (e.data.type === 'auth:failed') {
    refreshTokenAndReload(docId)
  }
})

async function refreshTokenAndReload(docId) {
  const { token } = await fetch(`/my-doc-token?doc=${docId}`).then(r => r.json())
  iframe.src = `${engineUrl}/embed?doc=${docId}&token=${token}&mode=edit`
}
```
...
鉴权打通后，下一步是 [前端 iframe 嵌入](./embedding.md) —— 构造 `/embed` URL。
