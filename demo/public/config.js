 ...
 *
 * 应用统一挂在 /oes 前缀下（与 vite base、容器 nginx location 一致）。
 *
 * 字段说明：
 *   apiBase    demo 后端（业务系统后端）地址。
 *              · 本地开发（npm run dev）：填 http://localhost:4001（直连后端，绕过 /oes 前缀）
 *              · Docker 部署：留空 ''（走同源，前端自动补 /oes 前缀，demo nginx 反代 /oes/api/）
...
window.__UMO_CONFIG__ = {
  ...
  // Vue Router 基础路径（固定 /oes，与 vite base 一致）
  routerBase: '/oes',
}
